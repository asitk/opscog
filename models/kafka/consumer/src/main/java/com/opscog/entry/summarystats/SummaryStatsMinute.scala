...
/** Created by prashun on 15/8/16.
  */
class SummaryStatsMinute(_customerid: String, _deploymentid: String)
  ...
  def getLastTimeSlot: Long = {
    ...
    val ts = {
      try {
        val query =
          s"SELECT ${SSMC.TS} FROM ${SSMC.TABLE_NAME} WHERE ${SSMC.CUSTOMER_ID} = '${customer_id}' AND ${SSMC.DEPLOYMENT_ID} = '${deployment_id}'" +
  ...
  def prepareTimelineQuery(
    ...
    val query =
      s"SELECT ${SSMC.TS}, ${SSMC.THERMAL} FROM ${SSMC.TABLE_NAME} WHERE " +
  ...
  def prepareDetailsQuery(
    ...
    val tsSet = getTimeRangeAsASetString(time_range, 1)
    val query =
      s"SELECT ${SSMC.TS}, ${SSMC.THERMAL}, ${SSMC.THERMAL_STATS} FROM ${SSMC.TABLE_NAME} " +
  ...
  def prepareConnectionQuery(
    ...
    val tsSet = getTimeRangeAsASetString(time_range, 1)
    val query =
      s"SELECT ${SSMC.TS}, ${SSMC.CONNECTIONS} FROM ${SSMC.TABLE_NAME} " +
        s"WHERE ${SSMC.CUSTOMER_ID} = '${customer_id}' AND ${SSMC.DEPLOYMENT_ID} = '${deployment_id}' " +
        s"AND ${SSMC.CLUSTER_ID} = '${cluster_id}' AND ${SSMC.HOST_NAME} = '${host_name}' " +
        s"AND ${SSMC.TS} IN (${tsSet}) ALLOW FILTERING"
    query
  ...
  def prepareServiceQuery(
    ...
    val tsSet = getTimeRangeAsASetString(time_range, 1)
    val query =
      s"SELECT ${SSMC.TS}, ${SSMC.SERVICES} FROM ${SSMC.TABLE_NAME} " +
        s"WHERE ${SSMC.CUSTOMER_ID} = '${customer_id}' AND ${SSMC.DEPLOYMENT_ID} = '${deployment_id}' " +
        s"AND ${SSMC.CLUSTER_ID} = '${cluster_id}' AND ${SSMC.HOST_NAME} = '${host_name}' " +
        s"AND ${SSMC.TS} IN (${tsSet}) ALLOW FILTERING"
    query

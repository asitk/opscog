...
import com.opscog.entry.Metrics._
import com.opscog.entry.SummaryState._
import com.opscog.entry.anomaly.Anomaly._
import com.opscog.entry.anomaly.Detections
...
/** Created by prashun on 3/8/16.
  */
object Predictions {
  case class Key(node: MetricsStats, prev_thermal: Int, current_thermal: Int)
  ...
  val rgrid: Map[Int, Int] = getSevGrades
    .map(x => {
  ...
  def correlate(): Unit = {
    ...
    val thermalStructs: Array[(Long, Int, String, String, String)] = {
      if (resultSet != null) {
        resultSet.asScala
          .map(row => {
            (
              row.getTimestamp("ts").getTime,
              row.getInt("thermal"),
              row.getString("thermal_stats"),
              row.getString("stats"),
              row.getString("connections")
            )
          })
          .toArray
      } else {
    ...
    thermalStructs.foreach(x => {
      ...
      stats.foreach(x => {
        ...
        val corr_factor =
          getMultiVariateParams(hmList, new TimeRange(start_ts, end_ts))
      ...
      if (wsList.nonEmpty) {
        ...
        val final_hmList =
          Detections.Generic.getHeatMapWithScoreFromWindowScore(final_ws)
        val start_ts = final_hmList.head.startTime
        val end_ts = start_ts + 3600000
        val corr_factor =
          getMultiVariateParams(final_hmList, new TimeRange(start_ts, end_ts))
        println(s"The probability = \n${corr_factor.probability.toString}")
        println(s"The coefficient = \n${corr_factor.coefficient.toString}")
  ...
  case class CORRFactor(
      probability: DenseMatrix[Double],
      coefficient: DenseMatrix[Double]
  ) extends Serializable {}

  def rankDependencies(
      thermal: Int,
      stats: List[Stats],
    ...
    val final_mat: DenseMatrix[Double] =
      (final_corr_factor.coefficient :* final_corr_factor.probability)

    val (r, c) = getRowCol(thermal, thermal)
    val inter = intermediates
      .filter(x => {
    ...
    for (i <- inter.indices) {
      val s = inter(i)._1
      val rank = i + 1
      val inter_mat: DenseMatrix[Double] =
        (inter(i)._2.coefficient :* inter(i)._2.probability)
      val share = inter_mat(0, c) / (final_mat(0, c) - constant_coeff(0, c))
      val contrib = new Contribution(rank, share)
      rankedStats ::= new Stats(
        s.customer_id,
        s.deployment_id,
        s.cluster_id,
        s.host_ip,
    ...
    rankedStats =
      rankedStats.sortWith(_.contribution.rank < _.contribution.rank)
    rankedStats
  }

  def solveMultiVariateCorrelation(
      intermediates: List[CORRFactor],
      finalCorr: CORRFactor,
      thermal: Int
  ): Unit = {
    var sum_prod: DenseMatrix[Double] =
      DenseMatrix.zeros(1, getSevGrades.length)
    ...
    val final_prod: DenseMatrix[Double] =
      finalCorr.coefficient :* finalCorr.probability
    ...
    println(
      s"The final coefficient_matrix = \n${finalCorr.coefficient.toString}"
    )

package com.opscog.entry.anomaly

import breeze.stats._
import com.opscog.entry.Metrics
import com.opscog.entry.Metrics.{DStats, MetricsStats}
import com.opscog.entry.NWObject._
import com.opscog.entry.NWSvcMetrics._
import com.opscog.entry.Stats._
import com.opscog.entry.anomaly.Anomaly._
import com.opscog.entry.infra.Node.NodeMap
import com.opscog.entry.infra.{Cluster, Node}
import com.opscog.util.CassandraDB._
import com.opscog.util.{JacksonWrapper, _}
...
/** Created by prashun on 29/8/16.
  */
class NetworkActivityDetection(
    _cluster_replicated: Boolean,
      ...
      score += k * {
        ...
        val o = {
          mode match {
            ...
            case Mode.BPF => {
              if (
                value < (stat.mean - 2 * stat.stddev) || value > (stat.mean + 2 * stat.stddev)
              ) 1
              else 0
            }
          }
  ...
  def setServiceStatsCounter(
      nwSvcStatsCounters: List[NWSvcStatsCounter]
  ): Unit = {
    val cassdb = getCassandraHandle
    nwSvcStatsCounters
      .filter(_.is_modified)
      .foreach(x => {
  ...
  def updateCounter(x: Array[NWGraph]): Unit = {
    ...
    val key =
      s"${z.to.customer_id}, ${z.to.deployment_id}, ${z.to.cluster_id}, ${z.to.host_ip}, ${z.svc_info.port}"
    val counter = {
    ...
    val avg_rsp_size_stats =
      updateStats(x.map(_.avg_rsp_size.toDouble), counter.avg_rsp_size_stats)
    val avg_ttfb_stats =
      updateStats(x.map(_.avg_ttfb.toDouble), counter.avg_ttfb_stats)
    val avg_ttlb_stats =
      updateStats(x.map(_.avg_ttlb.toDouble), counter.avg_ttlb_stats)
    val max_ttfb_stats =
      updateStats(x.map(_.max_ttfb.toDouble), counter.max_ttfb_stats)
    val max_ttlb_stats =
      updateStats(x.map(_.max_ttlb.toDouble), counter.max_ttlb_stats)
    val recv_bytes_stats =
      updateStats(x.map(_.recv_bytes.toDouble), counter.recv_bytes_stats)
    val sent_bytes_stats =
      updateStats(x.map(_.sent_bytes.toDouble), counter.sent_bytes_stats)
    val request_count_stats =
      updateStats(x.map(_.request_count.toDouble), counter.request_count_stats)
    val error_count_stats =
      updateStats(x.map(_.error_count.toDouble), counter.error_count_stats)
    val duration_stats =
      updateStats(x.map(_.duration.toDouble), counter.duration_stats)

    if (nw_svc_past_counters.contains(key)) {
      nw_svc_past_counters.remove(key)
    }

    nw_svc_past_counters += (key -> new NWSvcStatsCounter(
      z.ts,
      z.to,
      z.svc_info,
      avg_rsp_size_stats,
    ...
    if (resultSet != null) {
      val ctr = resultSet.asScala
        .map(row => {
          ...
          val to = JacksonWrapper.deserialize[Node.NodeMap](
            row.getString(SVC_STATS_COL.TO_INFO)
          )
          val svc_info = JacksonWrapper.deserialize[NWSvcInfo](
            row.getString(SVC_STATS_COL.SVC_INFO)
          )
          val avg_rsp_size_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.AVG_RSP_SIZE_STATS)
          )
          val avg_ttfb_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.AVG_TTFB_STATS)
          )
          val avg_ttlb_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.AVG_TTLB_STATS)
          )
          val max_ttfb_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.MAX_TTFB_STATS)
          )
          val max_ttlb_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.MAX_TTLB_STATS)
          )
          val recv_bytes_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.RECV_BYTES_STATS)
          )
          val sent_bytes_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.SENT_BYTES_STATS)
          )
          val request_count_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.REQUEST_COUNT_STATS)
          )
          val error_count_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.ERROR_COUNT_STATS)
          )
          val duration_stats = JacksonWrapper.deserialize[Metrics.DStats](
            row.getString(SVC_STATS_COL.DURATION_STATS)
          )
          var costly_requests = List[RequestStat]()
          var err_requests = List[RequestStat]()
          new NWSvcStatsCounter(
            ts,
            to,
            svc_info,
            avg_rsp_size_stats,
      ...
      .foreach(x => {
        ...
        for (i <- u.indices) {
          ...
          if (u(i).ts >= time_range.start && u(i).ts < time_range.end) {
            ...
            windowScore += (u(i).ts -> new WindowScore(
              score,
              u(i).ts,
              GRAIN,
              severity
        ...
        val qs = {
          ...
          if (qs.severity == sevGrades.head.grade) {
            new QuantizedSeverity(
              sevGrades(1).grade,
              qs.total_severity_duration,
              qs.single_max_severity_duration,
              qs.total_duration
            )
          } else {
            qs
  ...
  def workOnDistribution(
    ...
    val nsl =
      prepareNWGraphList(customer_id, deployment_id, arows, time_range).toArray
  ...
  def workOnDistribution(nsl: Array[NWGraph]): List[AnomalyCollection] = {
    ...
    nsl
      ...
      .foreach(x => {
        val key =
          s"${x.head.to.customer_id},${x.head.to.deployment_id},${x.head.to.cluster_id},255.255.255.255,${x.head.svc_info.port}"
    ...
    nsl
      ...
      .foreach(v => {
        ...
        val key =
          s"${x.head.to.customer_id},${x.head.to.deployment_id},${x.head.to.cluster_id},255.255.255.255,${x.head.svc_info.port}"
        val peer =
          nw_svc_counters.getOrElse(key, List()).sortWith(_.ts < _.ts).toArray
        val z = x.head
        detections :::= {
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                x.map(a => {
                  new Item(a.ts, a.avg_rsp_size.toDouble)
                }),
                peer.map(x => (x.ts, x.avg_rsp_size_stats)),
                mode
              )
            }
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.avg_ttfb_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.avg_ttlb_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.max_ttfb_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.max_ttlb_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.recv_bytes_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.sent_bytes_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.request_count_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.error_count_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
          ...
          val d = modes.map(mode => {
            ...
            val (windowScore, heatMap) = {
              getWindowScoreAndHeatMap(
                ...
                peer.map(x => (x.ts, x.duration_stats)),
                mode
              )
            }
            val ws = getWSFromWindowScore(windowScore)
  ...
  def getWindowScoreAndHeatMap(
    ...
    for (i <- x.indices) {
      val score = {
        if (simulate && Random.nextBoolean()) Random.nextDouble()
        else getScore(x, i, y, mode)
      }
      val severity = getSeverity(score)
  ...
  protected[anomaly] def writeNWSvcToKairosDB(
      arows: Array[Metrics.MetricsStats]
  ): Unit = {
    val customer_id = arows.head.customer_id
    val deployment_id = arows.head.deployment_id
    ...
    nsl
      ...
      .foreach(x => {
        ...
        val y = new NWGraph(
          ...
          from,
          z.svc_info,
          z.avg_rsp_size_stats.mean.toLong,
          z.avg_ttfb_stats.mean.toLong,
          z.avg_ttlb_stats.mean.toLong,
          z.max_ttfb_stats.mean.toLong,
          z.max_ttlb_stats.mean.toLong,
          z.error_count_stats.mean.toLong,
          z.request_count_stats.mean.toLong,
          z.sent_bytes_stats.mean.toLong,
          z.recv_bytes_stats.mean.toLong,
          Array(),
          Array(),
          z.ts,
          z.duration_stats.mean.toLong
        )
  ...
  def getObservationsFromAnomalyList(
      conn_type: Int,
    ...
    val d = {
      conn_type match {
        case ConnType.NODE => {
          ...
          detections
            .filter(_.node.plugin == NWPlgType.PLUGIN)
            .groupBy(x => {
              val z = x.node
              host_ip = z.host_ip
    ...
    val observable_ms = d.map(x => {
      ...
      val ms = new MetricsStats(
        ...
        "{}",
        "{}",
        JacksonWrapper.serialize(observation_map),
        ""
      )
      ms
    })
  ...
  def processNWConnections(
      connType: Int,
      plugin_level_list: Array[MetricsStats],
  ...
  def processServices(
      ...
      cluster_id: String,
      host_ip: String,
      anomalies: Array[AnomalyCollection],
      time_range: TimeRange
  ): List[ServiceInfo] = {
    /* Find out the services associated with the connType */
    val connType = {
      if (cluster_id == "*" && host_ip == "255.255.255.255") ConnType.DEPLOYMENT
      else if (cluster_id != "*" && host_ip == "255.255.255.255")
        ConnType.CLUSTER
      else if (cluster_id != "*" && host_ip != "255.255.255.255") ConnType.NODE
      else ConnType.NODE
    }

    val svc_list: List[NWSvcInfo] =
      getNWServices(customer_id, deployment_id, time_range)
        .map(_.nw_svc_info_list)
        .head
        .filter(x => {
          connType match {
    ...
    val observations =
      getObservationsFromAnomalyList(connType, anomalies.toList)
  ...
  def processNWConnections(
          ...
          connType match {
            case ConnType.CLUSTER => {
              hl.groupBy(x => {
                ...
                val from = x.from.copy(
                  x.from.customer_id,
                  x.from.deployment_id,
                  x.from.cluster_id,
                  "255.255.255.255",
                  "*",
                  x.from.scope,
      ...
      flist.foreach(x => {
        ...
        for (i <- crs.indices) {
          if (j < max_items && crs(i).req.length > 0) {
            val (index, found) =
              findAppropriateIndex(costliestReqStats, crs(i).req)
            if (found) {
              val reqStat: RequestStat = new RequestStat(
                costliestReqStats(index).req,
                costliestReqStats(index).count + crs(i).count,
                (costliestReqStats(index).ttfb * costliestReqStats(
                  index
                ).count + crs(i).ttfb * crs(i).count) / (costliestReqStats(
                  index
                ).count + crs(i).count),
                (costliestReqStats(index).ttlb * costliestReqStats(
                  index
                ).count + crs(i).ttlb * crs(i).count) / (costliestReqStats(
                  index
                ).count + crs(i).count)
              )
        ...
        for (i <- 0 until math.min(tes.length, 3)) {
          if (tes(i).count > 0 && j < max_items && tes(i).req.length > 0) {
            val (index, found) =
              findAppropriateIndex(topmostErrReqStats, tes(i).req)
            if (found) {
              val reqStat = new RequestStat(
                topmostErrReqStats(index).req,
                topmostErrReqStats(index).count + tes(i).count,
                (topmostErrReqStats(index).ttfb * topmostErrReqStats(
                  index
                ).count + tes(i).ttfb * tes(i).count) / (topmostErrReqStats(
                  index
                ).count + tes(i).count),
                (topmostErrReqStats(index).ttlb * topmostErrReqStats(
                  index
                ).count + tes(i).ttlb * tes(i).count) / (topmostErrReqStats(
                  index
                ).count + tes(i).count)
              )
    ...
    val diff = System.currentTimeMillis() - t1
    Log.getLogger.trace(
      s"End of processNWConnections - time taken = ${diff} ms"
    )

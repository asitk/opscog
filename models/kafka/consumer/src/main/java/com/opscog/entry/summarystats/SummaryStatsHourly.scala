package com.opscog.entry.summarystats
...
/** Created by prashun on 15/8/16.
  */
class SummaryStatsHourly(_customerid: String, _deploymentid: String)
    extends SummarizedStats {
  ...
  def getTimeSlots: List[Long] = {
    ...
    val timeNow = getTimeToNearestHour(
      Calendar
  ...
  def getLastTimeSlot: Long = {
    val query =
      s"SELECT ${SSHC.TS} FROM ${SSHC.TABLE_NAME} WHERE ${SSHC.CUSTOMER_ID} = '${customer_id}' " +
    ...
    try {
      resultSet.asScala
        .map(_.getTimestamp(SSHC.TS).getTime)
        .toList
        .head + 3600 * 1000
    } catch {
      case e: Exception => {
        try {
          ...
          val resultSet = cassdb.executeQuery(query)
          ts = resultSet.asScala
            .map(_.getTimestamp(Metrics.METRICS_STATS_COLS.TS).getTime)
            .toList
            .head
        } catch {
          case e: Exception => {
            Log.getLogger.error(
              s"${Metrics.METRICS_STATS_COLS.TABLE_NAME} is empty"
            )
            ts = 0L
          }
        }
  ...
  def updateTimeSlot(ssList: SummaryStats): Boolean = {
    val uhash =
      s"${ssList.customer_id},${ssList.deployment_id},${ssList.cluster_id},${ssList.host_ip},${ssList.host_name}"
  ...
  def prepareTimelineQuery(
    ...
    val query =
      s"SELECT ${SSHC.TS}, ${SSHC.THERMAL} FROM ${SSHC.TABLE_NAME} WHERE ${SSHC.CUSTOMER_ID} = '${customer_id}' " +
  ...
  def prepareDetailsQuery(
    ...
    val tsSet = getTimeRangeAsASetString(time_range, 60)
    val query =
      s"SELECT ${SSHC.TS}, ${SSHC.THERMAL}, ${SSHC.THERMAL_STATS} FROM ${SSHC.TABLE_NAME} " +
  ...
  def prepareSnapshotQuery(
    ...
    val tsSet = getTimeRangeAsASetString(time_range, 60)
    val query =
      s"SELECT ${SSHC.TS}, ${SSHC.THERMAL}, ${SSHC.THERMAL_STATS}, ${SSHC.STATS}, " +
  ...
  def prepareConnectionQuery(
    ...
    val tsSet = getTimeRangeAsASetString(time_range, 60)
    val query =
      s"SELECT ${SSHC.TS}, ${SSHC.CONNECTIONS} FROM ${SSHC.TABLE_NAME} " +
        s"WHERE ${SSHC.CUSTOMER_ID} = '${customer_id}' AND ${SSHC.DEPLOYMENT_ID} = '${deployment_id}' " +
        s"AND ${SSHC.CLUSTER_ID} = '${cluster_id}' AND ${SSHC.HOST_NAME} = '${host_name}' " +
        s"AND ${SSHC.TS} IN (${tsSet}) ALLOW FILTERING"
    query
  ...
  def prepareServiceQuery(
    ...
    val tsSet = getTimeRangeAsASetString(time_range, 60)
    val query =
      s"SELECT ${SSHC.TS}, ${SSHC.SERVICES} FROM ${SSHC.TABLE_NAME} " +
        s"WHERE ${SSHC.CUSTOMER_ID} = '${customer_id}' AND ${SSHC.DEPLOYMENT_ID} = '${deployment_id}' " +
        s"AND ${SSHC.CLUSTER_ID} = '${cluster_id}' AND ${SSHC.HOST_NAME} = '${host_name}' " +
        s"AND ${SSHC.TS} IN (${tsSet}) ALLOW FILTERING"
    query

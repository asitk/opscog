...
import com.opscog.entry.Metrics.MetricsStats
import com.opscog.entry.anomaly.Anomaly._
import com.opscog.entry.infra.Cluster
import com.opscog.entry.{Metrics, NWObject}
...
/** Created by prashun on 10/8/16.
  */
object Detections {
  def getPeerDetectionOnDistribution(
  ...
  def getThresholdDetectionOnDistribution(
      arows: Array[Metrics.MetricsStats],
      cluster_replicated: Boolean,
      time_range: TimeRange
  ): List[AnomalyCollection] = {
  ...
  def getErrorDetectionOnDistribution(
      arows: Array[Metrics.MetricsStats],
      cluster_replicated: Boolean,
      time_range: TimeRange
  ): List[AnomalyCollection] = {
  ...
  def getResourceExhaustionDetectionOnDistribution(
      arows: Array[Metrics.MetricsStats],
      cluster_replicated: Boolean,
      time_range: TimeRange
  ): List[AnomalyCollection] = {
  ...
  def getHeartbeatDetectionOnDistribution(
      arows: Array[Metrics.MetricsStats],
      cluster_map: Cluster.ClusterMap,
      time_range: TimeRange
  ): List[AnomalyCollection] = {
  ...
  def getServiceDetectionOnDistribution(
      arows: Array[Metrics.MetricsStats],
      cluster_map: Cluster.ClusterMap,
      time_range: TimeRange
  ): List[AnomalyCollection] = {
  ...
  def getNetworkActivityDetectionOnDistribution(
      arows: Array[Metrics.MetricsStats],
      cluster_replicated: Boolean,
      time_range: TimeRange
  ): List[AnomalyCollection] = {
  ...
  def writeNetworkActivityByService(
      arows: Array[Metrics.MetricsStats],
      cluster_replicated: Boolean,
      time_range: TimeRange
  ): Unit = {
  ...
  def writeToDB(
    ...
    val query_list = {
      ...
      detections.foreach(x => {
        ...
        val tmp_str =
          s"INSERT INTO ${ASC.TABLE_NAME}(${ASC.TS}, ${ASC.DURATION}, ${ASC.UHASH}, ${ASC.CUSTOMER_ID}, ${ASC.DEPLOYMENT_ID}, " +
  ...
  def getFromDB(
      ...
      thermal: Option[Int],
      uhash: Option[List[String]]
  ): List[AnomalyCollection] = {
    var detections = List[AnomalyCollection]()
    val query = {
      var str =
        s"SELECT ${ASC.TS}, ${ASC.DURATION}, ${ASC.CUSTOMER_ID}, ${ASC.DEPLOYMENT_ID}, ${ASC.CLUSTER_ID}, " +
      ...
      if (cluster_id.nonEmpty)
        str += s" AND ${ASC.CLUSTER_ID} = '${cluster_id.get}'"
      if (host_ip.nonEmpty) str += s" AND ${ASC.HOST_IP} = '${host_ip.get}'"
      if (host_name.nonEmpty)
        str += s" AND ${ASC.HOST_NAME} = '${host_name.get}'"
      ...
      if (classification.nonEmpty)
        str += s" AND ${ASC.CLASSIFICATION} = '${classification.get}'"
      if (class_type.nonEmpty)
        str += s" AND ${ASC.CLASS_TYPE} = '${class_type.get}'"
    ...
    resultSet.asScala.foreach(x => {
      ...
      val sc =
        new SeverityClassification(class_type, sub_type, _sc.qs, _sc.ws, _sc.hm)
  ...
  object Generic extends AnomalyTrait {
    ...
    def quantizeAnomalies(
      ...
      val qs = Generic.getQuantizedSeverity(
        windowScoreList,
        cluster_replicated,
        time_range
      )
      if (qs.severity != sevGrades.last.grade) {
        ...
        val message =
          Generic.getMessage(z.node, z.anomaly.subType, qs, cluster_replicated)

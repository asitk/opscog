...
import com.opscog.util.{JacksonWrapper, Log}
...
/** Created by prashun on 9/8/16.
  */
class ResourceExhaustionDetection(
  ...
  def willExhaustionOccur(
    ...
    do {
      ...
      if (dst >= finalDest) {
        ...
        if (
          matched && !confirm && (last_failed_val == last_last_failed_val && last_last_failed_val != 0L)
        ) {
          confirm = true
        }
      }
    ...
    if (matched) {
      Log.getLogger.trace(
        s"Resource exhaustion will occur at ${last_matched_val / 60} minutes"
      )
    }
  ...
  private def findETA(
    ...
    val x_mean =
      JacksonWrapper.deserialize[Metrics.DStats](x(pos).val_stats).mean
    willExhaustionOccur(avg_accel, avg_vel, highWaterMark - x_mean, x(pos).ts)
  }

  private def getScore(
      set: Array[Metrics.MetricsStats],
      pos: Int
  ): (Double, Long) = {
    var score: Double = 0

...
import com.opscog.entry.infra.{Cluster, Deployment, Node}
...
object DApi {
  def run(): Unit = {
    ...
    val a3 = Node.getTimelineMarkers(
      customer_id,
      deployment_id,
      cluster_id,
      host_ip,
    ...
    val b3 =
      Node.getDetails(customer_id, deployment_id, cluster_id, host_ip, tr)
    println(s"The node details are ${JacksonWrapper.serialize(b3)}")
    val c3 =
      Node.getSnapshot(customer_id, deployment_id, cluster_id, host_ip, tr)
    println(s"The node snapshot are ${JacksonWrapper.serialize(c3)}")
    val d3 =
      Node.getConnection(customer_id, deployment_id, cluster_id, host_ip, tr)
    println(s"The node connections are ${JacksonWrapper.serialize(d3)}")
    val e3 =
      Node.getService(customer_id, deployment_id, cluster_id, host_ip, tr)
    println(s"The node services are ${JacksonWrapper.serialize(e3)}")

    val m = NWObject.getNWServices(customer_id, deployment_id, tr)
    println(
      s"The deployment network svc details are ${JacksonWrapper.serialize(m)}"
    )

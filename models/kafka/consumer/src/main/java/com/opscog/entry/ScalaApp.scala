...
import com.opscog.entry.rawrecord._
import com.opscog.util.CassandraDB.Address
import com.opscog.util._
...
class StreamVarianceState(
  ...
  override def toString: String = {
    new String(
      "count=$count%ld, mean=${mean}%.4f, sumSquaresVar=$sumSquaresVar%.4f, welfordVar=$welfordVar%.4f, stddev=$stddev%.4f"
    )
  }
}
...
object StreamVarianceUpdateFunction {
  ...
  /** @param values
    *   The new data.
    * @param state
    *   The previous state from the last RDD iteration.
    * @return
    *   The new state: created from the previous state by updating it with the
    *   new data.
    */
  val updateFunc
      : (scala.collection.Seq[Double], Option[StreamVarianceState]) => Option[
        StreamVarianceState
      ] =
    (
        values: scala.collection.Seq[Double],
        state: Option[StreamVarianceState]
    ) => {
      if (values.isEmpty) {
        state
      } else {
        ...
        val sumSquaresVar = combinePrevCurrVars(
          prevSumSquaresVar,
          prevMean,
          prevCount,
          valsSumSquaresVariance,
          valsMean,
          valsCount
        )

        val valsWelfordVariance = welfordVariance(values)
        val welfordVar = combinePrevCurrVars(
          prevWelfordVar,
          prevMean,
          prevCount,
          valsWelfordVariance,
  ...
  def welfordVariance(values: scala.collection.Seq[Double]): Double = {
    ...
    def welfordRec(
      ...
      if (values.isEmpty)
        m2 / (n - 1) // Note we need n-1 for pop. variance here.
      else {
        val x = values.head
        val delta = x - mean
  ...
  /** Given the variances, means, and counts from two sets of data, find the
    * variance of the combined set. This algorithm is equivalent to the
    * algorithm given in Wikipedia's "Algorithms for calculating variance",
    * crediting Chan. Another equivalent implementation is in
    * spark.util.StatCounter.
    */
  def combinePrevCurrVars(
      prevVar: Double,
      prevMean: Double,
      prevCount: Long,
      currVar: Double,
    ...
    for (i <- 0 to 9) {
      ...
      val dataListSine: List[StreamVarianceData] = {
        for (j <- 0 to sineCount)
          yield {
            new StreamVarianceData(
              "sine",
              scala.math.sin(2 * scala.math.Pi * Random.nextDouble)
            )
          }
      }.toList
...
object ScalaApp extends App {
  ...
  def processVariance(o: (String, StreamVarianceState)): Unit = {
    ...
    val tagWelford =
      scala.collection.mutable.HashMap[String, String]("stat" -> "welford")
    val tagSumSq =
      scala.collection.mutable.HashMap[String, String]("stat" -> "sumsquares")
    val tagStd =
      scala.collection.mutable.HashMap[String, String]("stat" -> "stddev")

    // create a rawrecord and write it
    val rawWelford: RawRecord = {
      new RawRecord(
        RawRecordType.SYSTEM,
        plugin,
        classification + "_welford",
        target,
        hostip,
        hostname,
        clusterid,
  ...
  def persistOutliers(
    ...
    try {
      ...
      Log.getLogger.trace(
        s"${ts} ${customerid} ${category} ${hostip} ${hostname}  ${plugin} ${classification} ${otype} ${value} ${prb} ${cmpval}"
      )
    ...
    try {
      ...
      case e: Exception => {
        ...
        if (df.count() == 0) {
          ...
          df = rdd
            .toDF() // toDF("ip", "svc", "ipver", "proto","port", "interface")
  ...
  def testScalaCassandra(): Boolean = {
    ...
    val resultSet = cassdb.executeQuery(
      "select customer_id, deployment_id, ts, name, description, active from deployment_map"
    )
    if (resultSet != null) {
    ...
    if (!arg.isEmpty) {
      ...
      val cmdType = arg.head match {
        ...
        case "process-past-metrics-for-anomalies" => {
          ...
          Log.getLogger.trace(
            s"Going ahead with simulation = ${simulate.toString}"
          )
        ...
        case "process-metrics-for-anomalies" => {
          if (arg.length > 2) {
            ...
            Log.getLogger.debug(
              s"got values dur = ${n} and units = ${units} and simulate = ${simulate.toString}"
            )

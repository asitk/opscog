...
import com.opscog.entry.infra.Node.NodeDetailWithoutTimeRange
...
object Cluster extends Group {
  ...
  def getMaps(
    ...
    val query =
      s"SELECT ${CLUSTER_MAP_COLS.CUSTOMER_ID}, ${CLUSTER_MAP_COLS.DEPLOYMENT_ID}, ${CLUSTER_MAP_COLS.CLUSTER_ID}, ${CLUSTER_MAP_COLS.NAME}, " +
    ...
    val cluster_map = {
      if (resultSet != null) {
        val cluster_map = resultSet.asScala
          .filter(
            tr == null || _.getTimestamp(CLUSTER_MAP_COLS.TS).getTime <= tr.end
          )
          .filter(
            cust_id_filtered == _.getString(
              CLUSTER_MAP_COLS.CUSTOMER_ID
            ) || cust_id_filtered == null
          )
          .filter(
            deployment_id_filtered == _.getString(
              CLUSTER_MAP_COLS.DEPLOYMENT_ID
            ) || deployment_id_filtered == null
          )
          .filter(
            cluster_id_filtered == _.getString(
              CLUSTER_MAP_COLS.CLUSTER_ID
          ...
          .filter(
            active_filtered && _.getBool(
              CLUSTER_MAP_COLS.ACTIVE
            ) || !active_filtered
          )
          .map(row => {
            new ClusterMap(
              row.getString(CLUSTER_MAP_COLS.CUSTOMER_ID),
              row.getString(CLUSTER_MAP_COLS.DEPLOYMENT_ID),
              row.getString(CLUSTER_MAP_COLS.CLUSTER_ID),
              row.getString(CLUSTER_MAP_COLS.NAME),
              row.getString(CLUSTER_MAP_COLS.DESCRIPTION),
              row.getString(CLUSTER_MAP_COLS.ROLE),
              row.getBool(CLUSTER_MAP_COLS.ACTIVE),
              row.getBool(CLUSTER_MAP_COLS.REPLICATED),
              row.getTimestamp(CLUSTER_MAP_COLS.TS).getTime
            )
          })
          .toList
        cluster_map
  ...
  def getTimelineMarkers(
    ...
    if (clusterMap.length == 1) {
      ...
      new ClusterTimelineMarker(
        "get_cluster_timeline_markers",
        clusterMap.head.customer_id,
        clusterMap.head.deployment_id,
        clusterMap.head.cluster_id,
        clusterMap.head.name,
        clusterMap.head.description,
        clusterMap.head.active,
      ...
      throw new Exception(
        s"Customer_id = ${customer_id} with deployment_id = ${deployment_id}  with cluster_id = ${cluster_id} doesnt exist in cluster_map"
      )
      null
    }
  ...
  case class ClusterDetailWithoutNodeDetails(
      cluster_id: String,
      name: String,
      description: String,
      role: String,
  ...
  case class ClusterTimelineMarker(
      method: String,
      customer_id: String,
      deployment_id: String,
      cluster_id: String,
          ...
          if (thermalStructs.nonEmpty) {
            ...
            nodeMap.foreach(nm => {
              ...
              if (nd.nonEmpty) {
                ndList ::= {
                  val x = nd.head
                  new NodeDetailWithoutTimeRange(
                    x.host_name,
                    x.host_ip,
                    x.scope,
                    x.active,

...
import com.opscog.entry.anomaly._
import com.opscog.entry.infra.{Cluster, Customer, Deployment, Node}
...
object CreateRankingWindow {
  def processMetricsEveryNMinutes(n: Int, simulate: Boolean): Unit = {
    ...
    do {
      ...
      if (next_time_slot != 0) {
        ...
        if (diff > 0L) {
          for (i <- 0 until (diff / grain).toInt) {
            Log.getLogger.trace(
              s"Skipping time_slot = ${next_time_slot + i * grain}"
            )
          }
        }
      }
      ...
      val sleep_for_n_ms =
        (start + (behindInMinutes * 60 * 1000) - getUTCTimeNowInMillis())
      Log.getLogger.trace(
        s"The start time to process = ${start} and going to sleep for ${sleep_for_n_ms}\n"
      )
      ...
      customerMap.foreach(cm => {
        ...
        val deploymentMap =
          Deployment.getMaps(Some(cm.customer_id), None, active, None)
        deploymentMap.foreach(dm => {
          Log.getLogger.trace(s"Got deploymentmap ${dm}")
          val (detections, connections) = SummaryState.writeAnomaliesToDB(
            dm.customer_id,
            dm.deployment_id,
            tr,
            n
          ...
          Log.getLogger.trace(
            s"Computed all anomalies and connection summary for the deployment"
          )
          ...
          clusterMap.foreach(clm => {
            ...
            val nodeMap = Node.getMaps(
              Some(clm.customer_id),
              Some(clm.deployment_id),
              Some(clm.cluster_id),
              None,
              active,
              Some(tr)
            ...
            nodeMap.foreach(nm => {
              ...
              val sn = SummaryState.getNodeSnapshot(
                nm.customer_id,
                nm.deployment_id,
                nm.cluster_id,
                nm.host_ip,
              ...
              if (sn != null) {
                ss = new ssh.SummaryStats(
                  ...
                  nm.host_ip,
                  nm.host_name,
                  sn.thermal.summary.severity,
                  JacksonWrapper.serialize(sn.thermal.critInfo),
                  JacksonWrapper.serialize(sn.thermal.summary),
                  JacksonWrapper.serialize(sn.thermal.detail),
                  JacksonWrapper.serialize(sn.stats),
                  JacksonWrapper.serialize(sn.connections),
                  JacksonWrapper.serialize(sn.services)
                )
            ...
            val sn = SummaryState.getClusterSnapshot(
              clm.customer_id,
              clm.deployment_id,
              clm.cluster_id,
              tr,
          ...
          val sn = SummaryState.getDeploymentSnapshot(
            dm.customer_id,
            dm.deployment_id,
            tr,
            detections,
          ...
          if (ts >= nw_details_update_time) {
            Log.getLogger.trace(
              "Starting update of kafka queue meta with NW SVC Activities"
            )
            UpdateNWDetailsOnKafka.run(dm.customer_id, dm.deployment_id)
            Log.getLogger.trace(
              "Ending update of kafka queue meta with NW SVC Activities"
            )
  ...
  def processMetricsEveryNHours(n: Int, simulate: Boolean): Unit = {
    ...
    do {
      ...
      next_time_slot = start + grain
      val sleep_for_n_ms =
        (start + (behindInHours * 3600 * 1000) - getUTCTimeNowInMillis())
      Log.getLogger.trace(
        s"The start time to process = ${start} and going to sleep for ${sleep_for_n_ms}\n"
      )
      ...
      customerMap.foreach(cm => {
        ...
        deploymentMap.foreach(dm => {
          Log.getLogger.trace(s"Got deploymentmap ${dm}")
          val (detections, connections) = SummaryState.getAnomaliesByGrain(
            dm.customer_id,
            dm.deployment_id,
            None,
            None,

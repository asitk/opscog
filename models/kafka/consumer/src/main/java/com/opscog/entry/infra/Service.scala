...
object Service extends Group {
  ...
  def getMaps(
    ...
    val query =
      s"SELECT ${SERVICE_MAP_COLS.CUSTOMER_ID}, ${SERVICE_MAP_COLS.DEPLOYMENT_ID}, ${SERVICE_MAP_COLS.CLUSTER_ID}, " +
    ...
    val service_map = {
      if (resultSet != null) {
        val svc_map = resultSet.asScala
          .filter(
            customer_id.isDefined && _.getString(
              SERVICE_MAP_COLS.CUSTOMER_ID
            ) == customer_id.get || !customer_id.isDefined
          )
          .filter(
            deployment_id.isDefined && _.getString(
              SERVICE_MAP_COLS.DEPLOYMENT_ID
            ) == deployment_id.get || !deployment_id.isDefined
          )
          .filter(
            cluster_id.isDefined && _.getString(
              SERVICE_MAP_COLS.CLUSTER_ID
            ) == cluster_id.get || !cluster_id.isDefined
          )
          .filter(
            name.isDefined && _.getString(
              SERVICE_MAP_COLS.NAME
            ) == name.get || !name.isDefined
          )
          .filter(
            svc.isDefined && _.getInt(
              SERVICE_MAP_COLS.SVC
            ) == svc.get || !svc.isDefined
          )
          .filter(
            ipver.isDefined && _.getInt(
              SERVICE_MAP_COLS.IPVER
            ) == ipver.get || !ipver.isDefined
          )
          .filter(
            proto.isDefined && _.getInt(
              SERVICE_MAP_COLS.PROTO
            ) == proto.get || !proto.isDefined
          )
          .filter(
            port.isDefined && _.getInt(
              SERVICE_MAP_COLS.PORT
            ) == port.get || !port.isDefined
          )
          .filter(
            interface.isDefined && _.getString(
              SERVICE_MAP_COLS.INTERFACE
            ) == interface.get || !interface.isDefined
          )
          .map(row => {
            new ServiceMap(
              row.getString(SERVICE_MAP_COLS.CUSTOMER_ID),
  ...
  def setMap(item: ServiceMap): Boolean = {
    ...
    val uhash =
      s"${item.customer_id},${item.deployment_id},${item.cluster_id},${item.name}"
    val ttl = 2 * 86400

...
/** Created by prashun on 9/8/16.
  */
class PeerDetection(_cluster_replicated: Boolean, _time_range: TimeRange)
  ...
  private def getScore(set: Array[Metrics.MetricsStats], pos: Int): Double = {
    ...
    for (i <- pos - j until pos + 1) {
      score += k * {
        if (
          avoid_vals.contains(set(i).final_score) || set(
            i
          ).final_score.toString == "NaN"
        ) {
          0
        } else 1
      }
    ...
    if (fscore > 1.0) {
      throw new Exception(
        s"Cant get score > 1 on plugin = ${set.head.plugin} classification = ${set.head.classification} target = ${set.head.target} j = ${j}, pos = ${pos} k = ${k} max_score = ${max_score} and score = ${score}"
      )
    }
    fscore
  }

...
import com.opscog.injest.HighLevelProducer
...
object UpdateNWDetailsOnKafka {
  ...
  case class NWMessage(
      nodeDetails: Map[String, NWDetails],
      nwSvcInfoList: List[NWObject.NWSvcInfo]
  ) extends Serializable {
    override def toString(): String = {
      JacksonWrapper.serialize(this)
    }
  }
  case class NWDetails(
      customer_id: String,
  ...
  def run(customer_id: String, deployment_id: String): Unit = {
    ...
    Node
      ...
      .foreach(x => {
        nodeDetails += (x.host_ip -> new NWDetails(
          x.customer_id,
          x.deployment_id,
          x.cluster_id
        ))

package com.opscog.entry.infra

import java.util.Calendar

import com.opscog.entry.anomaly.Anomaly
import com.opscog.entry.infra.Cluster.ClusterDetailWithoutNodeDetails
import com.opscog.entry.summarystats._
import com.opscog.entry.{NWObject, SummaryState}
...
object Deployment extends Group {
  ...
  def getMaps(
    ...
    val query =
      s"SELECT ${DEPLOYMENT_MAP_COLS.CUSTOMER_ID}, ${DEPLOYMENT_MAP_COLS.DEPLOYMENT_ID}, ${DEPLOYMENT_MAP_COLS.NAME}, ${DEPLOYMENT_MAP_COLS.DESCRIPTION}, ${DEPLOYMENT_MAP_COLS.ACTIVE}, ${DEPLOYMENT_MAP_COLS.TS} FROM ${DEPLOYMENT_MAP_COLS.TABLE_NAME}"
    val resultSet = cassdb.executeQuery(query)
    val deployment_map = {
      if (resultSet != null) {
        val deployment_map = resultSet.asScala
          .filter(
            tr == null || _.getTimestamp(
              DEPLOYMENT_MAP_COLS.TS
            ).getTime <= tr.end
          )
          .filter(
            cust_id_filtered == _.getString(
              DEPLOYMENT_MAP_COLS.CUSTOMER_ID
            ) || cust_id_filtered == null
          )
          .filter(
            deployment_id_filtered == _.getString(
              DEPLOYMENT_MAP_COLS.DEPLOYMENT_ID
          ...
          .filter(
            active_filtered && _.getBool(
              DEPLOYMENT_MAP_COLS.ACTIVE
            ) || !active_filtered
          )
          .map(row => {
            new DeploymentMap(
              row.getString(DEPLOYMENT_MAP_COLS.CUSTOMER_ID),
              row.getString(DEPLOYMENT_MAP_COLS.DEPLOYMENT_ID),
              row.getString(DEPLOYMENT_MAP_COLS.NAME),
              row.getString(DEPLOYMENT_MAP_COLS.DESCRIPTION),
              row.getBool(DEPLOYMENT_MAP_COLS.ACTIVE),
  ...
  def setMap(item: DeploymentMap): Boolean = {
    ...
    val query =
      s"INSERT INTO ${DEPLOYMENT_MAP_COLS.TABLE_NAME}(${DEPLOYMENT_MAP_COLS.CUSTOMER_ID}, ${DEPLOYMENT_MAP_COLS.DEPLOYMENT_ID}, ${DEPLOYMENT_MAP_COLS.NAME}, ${DEPLOYMENT_MAP_COLS.DESCRIPTION}, ${DEPLOYMENT_MAP_COLS.ACTIVE}, ${DEPLOYMENT_MAP_COLS.TS}) " +
  ...
  case class DeploymentDetail(
      ...
      clusterDetailsList: List[ClusterDetailWithoutNodeDetails],
  ...
  case class DeploymentTimelineMarker(
      method: String,
      customer_id: String,
      deployment_id: String,
      name: String,
        ...
        if (thermalStructs.nonEmpty) {
          ...
          clusterMap.foreach(cm => {
            ...
            if (cld.nonEmpty) {
              ...
              cldList ::= {
                val x = cld.head
                new ClusterDetailWithoutNodeDetails(
                  x.cluster_id,
                  x.name,
                  x.description,
                  x.role,
    ...
    val dsList = deploymentMap
      .map(dm => {
        ...
        if (thermalStructs.nonEmpty) {
          ...
          val thermal_summary =
            JacksonWrapper.deserialize[List[Anomaly.HeatMapDetailWithScore]](
  ...
  def getTimelineMarkers(
      ...
      deployment_id: String,
      time_range: TimeRange
  ): DeploymentTimelineMarker = {
    val dm = Deployment.getMaps(
      Some(customer_id),
      Some(deployment_id),
      None,
      Some(time_range)
    ...
    if (dm.length == 1) {
      ...
      new DeploymentTimelineMarker(
        "get_deployment_timeline_markers",
        dm.head.customer_id,
        dm.head.deployment_id,
        dm.head.name,
        dm.head.description,
        dm.head.active,
      ...
      throw new Exception(
        s"Customer_id = ${customer_id} with deployment_id = ${deployment_id} doesnt exist in deployment_map"
      )
      null
    }
  }

...
import com.opscog.entry.infra.Cluster._
import com.opscog.entry.infra.Node
import com.opscog.entry.infra.Node._
...
/** Created by prashun on 9/8/16.
  */
class HeartbeatDetection(_cluster_map: ClusterMap, _time_range: TimeRange)
  ...
  def workOnDistribution(
    ...
    val arows =
      _arows.filter(_.plugin == "system").filter(_.classification == "uptime")
